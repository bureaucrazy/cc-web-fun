...
# Asks for all cities visited
arr=[]
loop do
  puts "Enter cities visited (type 'done' when finished): " if arr.count == 0 || arr.count % 10 == 0
  c=gets.chomp.strip
  break if c.downcase == "done"
  arr << c.title_case unless c.empty? || c.nil?
end

# Picks random city as infested.
infect = arr.sample
arr.delete(infect)
arr << "#{infect} <-- Zombie infestation alert!"
yourid[:cities] = arr.sort.uniq

puts "\nSecurity check complete!\n".green
puts "ATTENTION: #{yourid[:fname].bold} #{yourid[:lname].bold}!"
puts "You have visited the following #{yourid[:cities].count.to_s.red} cities since #{yourid[:age].to_s.red} years ago:"
# Displays numbered list of cities visited.
yourid[:cities].each {|v|
  print "#{yourid[:cities].index(v)+1}. "
# Makes the infested city colored.
  if v.include? "<--"
    puts "#{v.to_s.red}"
  else
    puts "#{v}"
...
print "Go to the ", yourid[:age] >= 18 ? "adults".red : "minors".red, " quarantine immediately.\n"
